...
Priority Classification (based on LEGITIMATE BUSINESS IMPACT):
- "high": REAL critical issues affecting service
  * Account locked/inaccessible
  * Payment failed multiple times
  * Data loss or corruption
  * Security breach (REAL, not test)
  * Service completely broken
  * Cannot complete core functionality
  
- "medium": Non-critical but valid problems
  * Bugs affecting experience
  * Feature requests
  * Service degradation
  * Slow performance
  * UI/UX issues
  
- "low": Invalid, spam, or minor issues
  * General feedback without issue
  * Questions
  * Suggestions
  * Insults without specific problem
  * Test messages ("test", "hack attempt", "hello", etc.)
  * Spam or gibberish
...
Examples:
- "fuck you" → priority: low, sentiment: negative (venting, no issue)
- "test" → priority: low, sentiment: neutral (test message)
- "hack attempt" → priority: low, sentiment: neutral (test/spam)
- "hello" → priority: low, sentiment: neutral (not a complaint)
...
- "Payment failed 3 times" → priority: high, sentiment: negative (critical financial)
...
- "My account is locked and I need access NOW" → priority: high, sentiment: negative
- "The new update is great!" → priority: low, sentiment: positive

IMPORTANT: Test messages, spam, and invalid complaints should ALWAYS be "low" priority.
