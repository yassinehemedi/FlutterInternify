...
class CvParserService {
  ...
  // Enable verbose debug logs for extraction flow
...
''';
  ...
  Future<String> _extractTextFromPdf(String path) async {
    try {
      if (_debug) print('cv_parser: starting PDF extraction for $path');
      // Try Syncfusion text extraction first (covers many PDFs reliably)
      try {
        if (_debug) print('cv_parser: trying Syncfusion PDF extractor');
        final bytes = await File(path).readAsBytes();
        final sfDoc = sf.PdfDocument(inputBytes: bytes);
        try {
          final extracted = sf.PdfTextExtractor(sfDoc).extractText();
          if (_debug) print('cv_parser: syncfusion extracted length=${extracted.length}');
          if (extracted.trim().isNotEmpty) {
            sfDoc.dispose();
            return extracted.trim();
          }
        } catch (e) {
          if (_debug) print('cv_parser: syncfusion extractor failed: $e');
          // ignore and fall through to other methods
        } finally {
          try { sfDoc.dispose(); } catch (_) {}
        }
      } catch (e) {
        if (_debug) print('cv_parser: syncfusion attempt error: $e');
      }

      final doc = await PdfDocument.openFile(path);
      final buffer = StringBuffer();

      // First try: use any available text extraction API on the page object
      ...
      // If we found text, return it. Otherwise fall back to rendering pages and OCRing them.
      var result = buffer.toString().trim();
      if (_debug) print('cv_parser: page-based extraction total length=${result.length}');
      ...
      if (_debug) print('cv_parser: no text found, starting OCR fallback for ${doc.pagesCount} pages');
      // OCR fallback: render each page as an image and run ML Kit OCR
      for (int i = 1; i <= doc.pagesCount; i++) {
        final page = await doc.getPage(i);
        Uint8List? imgBytes;
        try {
          final rendered = await (page as dynamic).render(width: 1200);
          imgBytes = (rendered as dynamic).bytes as Uint8List?;
        } catch (e) {
          if (_debug) print('cv_parser: render page $i failed: $e');
          imgBytes = null;
        }
        try {
          await page.close();
        ...
        if (imgBytes != null && imgBytes.isNotEmpty) {
          if (_debug) {
            try {
              ...
              final tmpPath = '${tmpDir.path}/pdf_page_${DateTime.now().millisecondsSinceEpoch}_$i.png';
              await File(tmpPath).writeAsBytes(imgBytes);
              print('cv_parser: saved rendered page to $tmpPath');
            } catch (e) {
              print('cv_parser: failed saving rendered image: $e');
            }
          }
          final ocrText = await _ocrImageBytes(imgBytes);
          if (_debug) print('cv_parser: ocr for page $i length=${ocrText.length}');
          if (ocrText.isNotEmpty) buffer.writeln(ocrText);
        }
      }
      ...
      if (_debug) print('cv_parser: final extracted (OCR) length=${finalText.length}');
      return finalText;
    } catch (e) {
      if (_debug) print('cv_parser: PDF extraction failed: $e');
  ...
  Future<Uint8List?> renderFirstPageAsImage(String path, {int width = 1200}) async {
    try {
      ...
      if (img != null) {
        ...
        if (_debug && bytes != null && bytes.isNotEmpty) {
          try {
            final tmpDir = Directory.systemTemp;
            final tmpPath = '${tmpDir.path}/render_first_page_${DateTime.now().millisecondsSinceEpoch}.png';
            await File(tmpPath).writeAsBytes(bytes);
            print('cv_parser: saved first-page image to $tmpPath');
          } catch (_) {}
        }
  ...
  Future<String> _ocrImageBytes(Uint8List imgBytes) async {
    if (_debug) print('cv_parser: running OCR on image bytes length=${imgBytes.length}');
    final tmpDir = Directory.systemTemp;
    ...
    try {
      ...
      if (_debug) print('cv_parser: OCR extracted length=${extracted.length}');
      try { await tmpFile.delete(); } catch (_) {}
      await textRecognizer.close();
      return extracted;
    } catch (e) {
      if (_debug) print('cv_parser: OCR failed: $e');
      try { await tmpFile.delete(); } catch (_) {}
      await textRecognizer.close();
      return '';
    }
