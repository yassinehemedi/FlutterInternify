...
class UserService {
  ...
  /// Delete multiple users by their IDs. Returns number of user rows deleted.
  Future<int> deleteUsersByIds(List<int> userIds) async {
    if (userIds.isEmpty) return 0;
    final db = await _db;
    int deleted = 0;
    await db.transaction((txn) async {
      for (final id in userIds) {
        final userList = await txn.query('users', where: 'id = ?', whereArgs: [id], limit: 1);
        if (userList.isEmpty) continue; // skip missing
        final role = userList.first['role'] as String?;
        if (role == 'job_seeker') {
          await txn.delete('job_seekers', where: 'userId = ?', whereArgs: [id]);
        } else if (role == 'enterprise') {
          await txn.delete('enterprises', where: 'userId = ?', whereArgs: [id]);
        }
  ...
  /// Delete all users and related records. Returns number of user rows deleted.
  /// Use with caution - this will permanently remove all users from the DB.
  Future<int> deleteAllUsers() async {
    final db = await _db;
    return await db.transaction((txn) async {
      // Clear dependent tables first to avoid FK issues (if any)
      await txn.delete('job_seekers');
      await txn.delete('enterprises');
      final deleted = await txn.delete('users');
      return deleted;
    });
  }
