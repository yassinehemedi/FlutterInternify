...
class OfferService {
  ...
  Future<Offer?> createOffer(Offer offer) async {
    final db = await _db;
    // Enforce that only enterprise users can create offers
    try {
      final rows = await db.query('users',
          columns: ['role'], where: 'id = ?', whereArgs: [offer.userId]);
      final role = rows.isNotEmpty ? (rows.first['role'] as String?) : null;
      if (role == null || role.toLowerCase() != 'enterprise') {
        throw Exception('Only enterprise users can create offers');
      }
    } catch (e) {
